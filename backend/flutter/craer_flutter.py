...
def generar_main_dart(estructura, base_flutter):
    ...
    with open(archivo, "w", encoding="utf-8") as f:
        ...
        for tabla_id, tabla in estructura['tablas'].items():
            nombre_clase = tabla['nombre'].capitalize()
            f.write(f"import 'screens/{nombre_clase}Screen.dart';\n")
...
def generar_screen_dart(tabla, base_screens, todas_tablas):
    ...
    nombre_clase = tabla['nombre'].capitalize()
    nombre_archivo = f"{nombre_clase}Screen.dart"
    ...
    with open(archivo, "w", encoding="utf-8") as f:
        ...
        fks_importadas = set()
        for atributo in tabla['atributos']:
            if atributo.get('tabla_padre') and atributo['tabla_padre'] not in fks_importadas:
                ...
                fks_importadas.add(atributo['tabla_padre'])
        
        f.write("\n")
        
        # ===== CLASE PRINCIPAL =====
        f.write(f"class {nombre_clase}Screen extends StatefulWidget {{\n")
        f.write(f"  const {nombre_clase}Screen({{Key? key}}) : super(key: key);\n\n")
        f.write("  @override\n")
        ...
        # DELETE ITEM
        if tiene_pk_compuesta:
            params = ', '.join([f"int {pk['atributo_padre'].lower()}" for pk in tabla['pk']])
        ...
        if tiene_pk_compuesta:
            update_args = ', '.join([f"item.{pk['atributo_padre'].lower()}!" for pk in tabla['pk']])
            f.write(f"              await _service.update({update_args}, newItem);\n")
        else:
            pk = tabla['pk'][0] if tabla['pk'] else None
        ...
        f.write("            _loadItems();\n")
        f.write("            if (mounted) {\n")
        f.write("              Navigator.pop(context);\n")
        f.write("              ScaffoldMessenger.of(context).showSnackBar(\n")
        f.write("                SnackBar(content: Text(item == null ? 'Creado' : 'Actualizado')),\n")
        f.write("              );\n")
        f.write("            }\n")
        f.write("          } catch (e) {\n")
        f.write("            if (mounted) {\n")
        ...
        f.write(f"        title: const Text('Detalles'),\n")
        ...
        if tiene_pk_compuesta:
            for pk in tabla['pk']:
                ...
                f.write(f"            Text('{label}: \\${{item.{pk['atributo_padre'].lower()}}}'),\n")
        else:
            if tabla['pk']:
                pk = tabla['pk'][0]
                label = pk['nombre'].capitalize()
                f.write(f"            Text('{label}: \\${{item.{pk['nombre'].lower()}}}'),\n")
        ...
        for atributo in tabla['atributos']:
            ...
            if es_fk:
                # Mostrar nombre de la tabla relacionada si existe
        ...
        f.write("          : _error != null\n")
        f.write("              ? Center(child: Text(_error!))\n")
        f.write("              : _items.isEmpty\n")
        f.write("                  ? const Center(child: Text('No hay datos'))\n")
        ...
        # Título del ListTile (primer atributo no-FK)
        ...
        f.write("                                        title: const Text('Confirmar'),\n")
        f.write("                                        content: const Text('¿Eliminar?'),\n")
        f.write("                                        actions: [\n")
        f.write("                                          TextButton(\n")
...
def generar_formulario_dart(f, tabla, nombre_clase, todas_tablas):
    ...
    f.write(f"  const {nombre_clase}FormDialog({{Key? key, this.item, required this.onSave}}) : super(key: key);\n\n")
    f.write("  @override\n")
    ...
    # Controllers para cada campo (excepto PKs auto_increment)
    for atributo in tabla['atributos']:
        es_fk = atributo.get('tabla_padre') is not None
        if not es_fk:
    ...
    for atributo in tabla['atributos']:
        ...
        if not es_fk:
            nombre_campo = atributo['nombre'].lower()
            f.write(f"    _{nombre_campo}Controller = TextEditingController(text: widget.item?.{nombre_campo}?.toString() ?? '');\n")
    ...
    for atributo in tabla['atributos']:
        if atributo.get('tabla_padre'):
            ...
            f.write("      });\n")
            f.write("    } catch (e) {\n")
            f.write("      print('Error loading {}: \\$e');\n".format(tabla_padre_lower))
            f.write("    }\n")
            f.write("  }\n\n")
    ...
    for atributo in tabla['atributos']:
        ...
        if es_fk:
            ...
            f.write("              DropdownButtonFormField<")
            f.write(f"{tabla_padre_cap}>(\n")
            f.write(f"                value: _selected{tabla_padre_cap},\n")
            f.write(f"                decoration: InputDecoration(labelText: '{label}'),\n")
            ...
            for tid, t in todas_tablas.items():
            ...
            if tipo_dato in ['integer', 'smallint', 'bigint', 'float', 'decimal']:
                ...
                f.write("                keyboardType: TextInputType.datetime,\n")
    ...
    for atributo in tabla['atributos']:
        ...
        if es_fk:
            ...
            if tipo_dato in ['integer', 'smallint', 'bigint']:
                f.write(f"                {nombre_campo}: int.tryParse(_{nombre_campo}Controller.text),\n")
            elif tipo_dato in ['float', 'decimal']:
                f.write(f"                {nombre_campo}: double.tryParse(_{nombre_campo}Controller.text),\n")
